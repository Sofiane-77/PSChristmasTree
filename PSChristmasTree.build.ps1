...
$loadedInvokeBuild = Get-Module -Name InvokeBuild | Sort-Object Version -Descending | Select-Object -First 1

if ($null -eq $loadedInvokeBuild -or $loadedInvokeBuild.Version -lt $minimumInvokeBuildVersion) {
	$detectedVersion = if ($null -eq $loadedInvokeBuild) { 'not loaded' } else { $loadedInvokeBuild.Version.ToString() }

	throw @"
Unsupported InvokeBuild version detected: $detectedVersion.
PSChristmasTree.build.ps1 requires InvokeBuild >= $minimumInvokeBuildVersion to avoid known local build failures (for example 'ProgressAction' metadata errors).
...
Enter-Build {
	...
	$Script:ModuleBuildZipPath = "$BuildOutputPath\$Configuration\$ModuleName\"
	$Script:ModuleBuildZipFile = Join-Path -Path $ModuleBuildZipPath -ChildPath "$ModuleName-v$ModuleVersionString.zip"
	$Script:TestResultsPath = Join-Path -Path $BuildOutputPath -ChildPath 'TestResults'
	$Script:PesterResultFile = Join-Path -Path $TestResultsPath -ChildPath 'pester-results.xml'
...
task Analyze {
	& (Join-Path -Path $BuildRoot -ChildPath 'tools/Invoke-PSChristmasTreeAnalyzer.ps1')
}

task Test {
	Write-Verbose -Message "Running Pester Tests"
	& (Join-Path -Path $BuildRoot -ChildPath 'tools/Invoke-PSChristmasTreeTests.ps1') -Output Detailed -ResultFile $PesterResultFile
}

task Format {
	& (Join-Path -Path $BuildRoot -ChildPath 'tools/Invoke-PSChristmasTreeFormatter.ps1')
}

task FormatCheck {
	& (Join-Path -Path $BuildRoot -ChildPath 'tools/Invoke-PSChristmasTreeFormatter.ps1') -Check
}

task DocsCheck {
	& (Join-Path -Path $BuildRoot -ChildPath 'tools/Test-PSChristmasTreeDocsSync.ps1')
}

task Coverage {
	& (Join-Path -Path $BuildRoot -ChildPath 'tools/Test-Coverage.ps1')
}

task Benchmark {
	& (Join-Path -Path $BuildRoot -ChildPath 'tools/Benchmark-PSChristmasTree.ps1')
}

task TestWindowsPowerShell -if($IsWindows) {
	$windowsPowerShell = Get-Command -Name 'powershell.exe' -ErrorAction Stop
	& $windowsPowerShell.Source -NoProfile -ExecutionPolicy Bypass -File (Join-Path -Path $BuildRoot -ChildPath 'tools/Invoke-PSChristmasTreeTests.ps1') -Output Detailed -ResultFile $PesterResultFile
}
...
task Zip -if($Configuration -eq "Release") {
	$compress = @{
		Path             = $ModuleBuildZipPath
		CompressionLevel = "Optimal"
		DestinationPath  = $ModuleBuildZipFile
		Force            = $true
	}
...
# CI task for Release workflow: build + artifacts only, no publish, no version bump
task CIBuild Init, Analyze, Test, DocsCheck, Build, Zip, Docs, Clean

# CI task for Publish workflow: full release including PSGallery publish, no version bump
task CIRelease Init, Analyze, Test, DocsCheck, Build, Zip, Docs, Publish, Clean

task . Init, Analyze, Test, Build, Zip, Docs, Publish, Clean, GenerateNewModuleVersion
